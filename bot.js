...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['*help','Type *help','bot by Jamel_syr','اكتب * للمساعدة','];
    var i = -1;
    var j = 0;
 ...
 client.on('message', message => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .setColor('RANDOM')
            .setTitle('``INFO Jamel_syr Community ©`` ')
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'Jamel_syr Community ©\'s translator',
...
.addField('`Your Name`',`__jamel__<@` + `${z.id}` + `>**`, true)
.addField('`ID`', "**"+ `${z.id}` +"**",true)
.addField('`Status`','**'+y+'**' , true)
.addField('`Acount Type`',"playkey44@gmail.com"+ w + "**",true)    
.addField('`Your Tag`',"#7709" +  `${z.discriminator}**`,true)
...
client.on('guildCreate', guild => {
  ...
  .setDescription(`**
  New Server Add jamel_syr Community © ?
...
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=435392018693488641&scope=bot&permissions=2146958591')
  .setDescription(`**
  Server Kicked jamel_syr Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
